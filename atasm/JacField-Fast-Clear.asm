...
addtab	= base+$1000		;Height add tab.
...
*	= sound
	.incbin "JacField-Great$3800.snd"
*	= chr
	.incbin "JacField.chr"
*	= field
	.incbin "JacField.dat"
...
Aplot1	ldx #0
Aplotb	ldy berg,x		;Get additional y-position.
	lda bhigh,y		;Get high-byte of the color table that corresponds to the y-position.
	sta Aplot_btab+2	;Set the right btab.
	tya
Aplot2	adc field+$100,x	;Ytab from field, odd page.
	tay
...
Aplot4	sta oldhi1,x
	lda llo,y		;Y_pos.
Aplot6	ldy field+$000,x	;Xtab from field, even page.
	adc otab,y		;X_pos_(byte).
	Zta Aplotp+1
	Zta Aplotp+4
Aplot8	sta oldlo1,x
Aplot_btab
	lda btab1,y		;X_pos_(pixel).
Aplotp	ora $ffff		;ORA (P1)
	sta $ffff
	inx			;Next_Point.
	.byte $d0, $cd		;bne Aplotb
	rts
...
;	Zta Aplotp+1
;Aplot8	sta oldlo1,x
;Aplot_btab	lda btab1,y		;X_pos_(pixel).
;	ldy #0
;Aplotp	ora $ffff		;ORA (P1)
...
scrres	.byte " this demo is a special contribution to the latest "
	...
	.byte "this finished in time ... because somtimes x-max parties "
...
clear1
	.rept 256
	ldy oldlo1+[[*-clear1]/10]
	ldx oldhi1+[[*-clear1-3]/10]
	stx p1+1
	sta (p1),y
	.endr
	rts
...
clear2
	.rept 256
	ldy oldlo2+[[*-clear2]/10]
	ldx oldhi2+[[*-clear2-3]/10]
	stx p1+1
	sta (p1),y
	.endr
	rts
